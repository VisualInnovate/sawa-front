...
const shift24=ref({})
const shift = ref({title: null,timeZone :Intl.DateTimeFormat().resolvedOptions().timeZone , days: [
]});
...
const fetchData =()=>{
   ...
   axios.get(`/api/shifts/${route.params.id}`)
      .then((res) => {
        ...
        shift.value.timeZone=Intl.DateTimeFormat().resolvedOptions().timeZone;
