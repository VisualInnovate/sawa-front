...
const edit=(id)=>{
  router.push({name:'shift-update',params:{'id':id} })
}
...
const deleteAction = () => {
  axios
    .delete(`/api/shifts/${confir_id.value}`)
    .then((res) => {
      console.log(res.data)
    ...
    <div class="col-12">
      <div class="page">
        <Toolbar>
          <template #start>
            <div class="my-2">
            <Button v-can="'shifts create'"  :label='$t("create_button")' icon="pi pi-plus" class="mr-2" @click="openNew"></Button>
<!--              <Button-->
<!--                label="Delete"-->
<!--                icon="pi pi-trash"-->
...
<!--            />-->
            <Button v-can="'shifts list'"  :label='$t("export")' icon="pi pi-upload" @click="exportCSV($event)" severity="secondary" variant="outlined" />
          </template>
        </Toolbar>
      ...
      <div>
        ...
        >
          <template #header>
            <div class="flex w-full  justify-between align-items-center">
              <h5 class="page-title">{{ $t("shift") }}</h5>
             <div>
          ...
          <Column selection-mode="multiple" header-style="width: 3rem"></Column>
           ...
           <Column field="shift_days" :header='$t("shift_days")' :sortable="true" header-style="width:14%; min-width:15rem;" class="ltr:text-justify">
            <template #body="slotProps">
              <span v-for="days in slotProps.data.shift_days">{{ days.day +' ' +'&'}}  </span>
            </template>
           </Column> 
        ...
        <Dialog v-model:visible="createdialog" :style="{ width: '450px' }" :header='$t("submit")' :modal="true">
            <div class="flex flex-column gap-2">
                  ...
                  <MultiSelect v-model="employee.users_ids"  required  option-value="id" filter :options="allusers" optionLabel="name" :placeholder='$t("users")' class="w-full" />
