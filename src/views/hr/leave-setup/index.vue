      ...
      <div style="" class="shadow-xl ">
        ...
        <Dialog v-model:visible="createdialog" :style="{ width: '450px' }" :header='$t("submit")' :modal="true">
            ...
            <div class="flex flex-column gap-2">
                ...
                <InputText required class="bg-[#f7f5f5] text-center" v-model="employee.title" :placeholder='$t("skill_name")' />
                <div class="mt-1 mb-5 text-red-500" v-if="error?.title">{{ error.title[0] }}</div>
            </div>
            <div  class="flex flex-column gap-2">
                  <label for="username">{{ $t('days') }}</label>
                  <InputNumber required class="bg-[#f7f5f5]" v-model="employee.days" :placeholder='$t("days")' />
