...
<template>
  <v-card class="mx-auto pa-12 pb-8 bg-slate-50" elevation="8">
    ...
    <form @submit.prevent="update">
      <div class="">
                 <div ><img onclick="document.getElementById('filr').click()" class="m-auto rounded-full" style="width: 150px ;height: 150px;" :class="{ 'p-invalid': submitted && !usersdata.image}" :src=" usersdata.image" >
...
const update=()=>{
  ...
  console.log(usersdata.value.name)
    ...
    axios
    .post(`/api/users/${router.currentRoute._value.params.id}/edit`,body)
    .then((res) => {
...
const getone=()=>{
  ...
  axios.get(`/api/users/${router.currentRoute._value.params.id}`).then((res)=>{
    
     times.value=res.data.business_hours
    ...
    for (let i =0 ; i < res.data.skills.length; i++) {
        ...
        usersdata.value.skills.push(res.data.skills[0].id);
