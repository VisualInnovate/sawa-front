...
const REQUIRED = [
  ...
  "addtional_phone_degree", "conversion_type", "child_doctor", "child_parents_problems", "parents_priorities",
];
  ...
  <div class="page">
    ...
    <form v-else class="booking-form" novalidate data-no-request-spinner @submit.prevent="submit">
      ...
      <section class="surface-card form-section">
        ...
        <div class="form-grid">
          <div v-for="key in ['requester_name', 'relative_degree', 'requester_phone', 'addtional_phone', 'addtional_phone_owner', 'addtional_phone_degree', 'conversion_type', 'child_doctor']"
            :key="key" class="field required">
            <label :for="`booking-${key}`">{{ $t({
              requester_name: 'اسم ولي امر  مقدم الطلب', relative_degree: 'درجه قرابته للطفل ', requester_phone: 'رقم هاتف ولي الامر',
              addtional_phone: 'رقم هاتف اضافي', addtional_phone_owner: 'مالك الرقم الاضافي', addtional_phone_degree: 'درجه قرابته بالطفل',
              conversion_type: 'مصدر التحويل', child_doctor: 'الطبيب الخاص بالطفل',
            }[key]) }}</label>
            <InputText :id="`booking-${key}`" v-model="details[key]" fluid :invalid="Boolean(error(`details.${key}`))" />
            <small v-if="error(`details.${key}`)" class="field-error">{{ error(`details.${key}`) }}</small>
          </div>
        </div>
      </section>
      ...
      <section class="surface-card form-section">
        ...
        <div class="form-grid">
          ...
          <div v-if="details.child_problem === 5" class="field required">
          ...
          <div v-if="details.child_aids === 1" class="field required">
        ...
        <div class="text-fields">
          <div class="field required">
            ...
            <Textarea id="booking-parents-problems" v-model="details.child_parents_problems" rows="4" autoResize fluid
          ...
          <div class="field required">
            ...
            <Textarea id="booking-priorities" v-model="details.parents_priorities" rows="4" autoResize fluid
