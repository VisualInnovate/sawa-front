<template>
    <div class="sawa-card p-[1%] bg-slate-50">
        <Toolbar>
          <template #start>
            <div class="my-2">
                <Select @update:model-value="getTimes" :placeholder='$t("employee_name")' v-model="event.employee_id"  option-value="id" filter :options="employees" optionLabel="name"  />
          ...
          <div>
            ...
            <div v-if="event.sub" class="flex flex-column gap-2 py-1">
                 <div class="flex text-center m-auto">
                  <label class=" text-start text-base my-auto" v-for="day in days_week">{{ day.name + ',' }}</label>
                  <label class=" text-start text-base my-auto">{{ $t("on_days") }}</label>
                  <label class=" text-start my-auto" for="username">{{ $t(' التكرار') }}</label>
                 </div>
      ...
      <Dialog v-model:visible="updateevent" id="modal" modal :header="modal_text" :style="{ width: '40vw' }">
        <form @submit.prevent="updateEvent">
          <div>
            ...
            <div class="flex gap-2 my-2">
                <ToggleSwitch v-model="repeat" /> 
                  <span class="px-2 text-yellow-300"> {{ $t('انتبة سوف يتم حذف او تعديل جميع الاحداث المكررة') }}</span>
            </div>
         
           <div class="flex ">
            <Button v-can="'working hours edit'" type="submit" class="mt-3"  icon="pi pi-pencil"      @click="updateEvent " severity="info" v-tooltip.top="$t('edit')" :aria-label="$t('edit')" />
            <Button v-can="'working hours delete'" type="submit" class="mt-3"  icon="pi pi-trash"  @click="deleteEvent " severity="danger" v-tooltip.top="$t('delete')" :aria-label="$t('delete')" />
           </div>
            <!-- <Button  label="Update" :loading="loading" @click="updateEvent" />
            <Button      label="Delete"  :loading="loading"  @click="deleteEvent" severity="danger" />-->
