<template>
  <div>
    ...
    <div class="sawa-card">
      ...
      >
        <div
          class="shadow-md bg-slate-100 rounded-sm p-4 grid grid-cols-2"
        ...
        >
          <div>
            ...
            <div class="flex py-2">
              ...
              <p class="text-xl px-1 my-auto">
                {{ evalu.specialist?.name || evalu.doctor?.name || "-" }}
              </p>
            </div>
            <div class="flex py-2">
              <h3 class="my-auto font-bold">{{ $t(" حالة التقييم") }} :</h3>
              <p v-if="evalu.status == 1" class="text-xl px-1 my-auto">
                {{ $t("status_finished") }}
              </p>
              <p v-else-if="evalu.status == 0" class="text-xl px-1 my-auto">
                {{ $t("status_under_evaluation") }}
              </p>
              <p v-else class="text-xl px-1 my-auto">-</p>
            </div>
          </div>
          <div class="text-center">
            <Button
              ...
              @click="go_evaluate(evalu.id, evalu.type, evalu.child_id, evalu)"
              class="m-auto"
              >{{ $t("evaluation_results") }}</Button
            >
            <Button
              v-can="'evaluations delete'"
              icon="pi pi-trash"
              @click="deleteevalution(evalu.id, evalu.child_id)"
              class="m-auto"
              severity="danger"
      ...
      <div v-else class="p-6 text-center text-gray-600">
        {{ $t("no_evaluations_yet") }}
      </div>

      <div v-if="total_pages > 1" class="w-full pb-2 text-center">
        <div class="card">
          <Paginator
    ...
    >
      <div class="flex align-items-center justify-content-center">
        ...
        <span>{{ $t("هل انت متاكد من ازالة هذا العنصر") }} ?</span>
      </div>
      <template #footer>
        <Button
          :label="$t('no')"
    ...
    >
      <form @submit.prevent="create" class="">
        <div class="flex flex-column gap-2 py-1">
          ...
          <Select
            @update:model-value="getdoctor_evalte"
            required
            v-model="evalate.evaluation_type"
            option-value="id"
            filter
            :options="evaluate_types"
            optionLabel="name"
            class="w-full"
            :class="{ 'p-invalid': submitted && !evalate.evaluation_type }"
          />
        </div>
        <div v-if="evalate.evaluation_type" class="flex flex-column gap-2 py-1">
          ...
          <Select
            @update:model-value="getDays"
            required
            v-model="evalate.specialist_id"
            option-value="id"
            filter
            :options="doctors"
            optionLabel="name"
            class="w-full"
            :class="{ 'p-invalid': submitted && !evalate.specialist_id }"
          />
        </div>
        <div v-if="evalate.specialist_id" class="flex flex-column gap-2">
          ...
          <DatePicker
            ...
            :disabledDays="filteredDays"
            style="width: 100%"
            showButtonBar
            v-model.number="evalate.date"
            showIcon
            :class="{ 'p-invalid': submitted && !evalate.date }"
            :minDate="maxDate"
          />
          <div class="mt-1 mb-5 text-red-500" v-if="error?.date">
            {{ error.date[0] }}
          </div>
        </div>
        <div v-if="evalate.date" class="flex flex-column gap-2 py-1">
          ...
          <Select
            required
            v-model="evalate.Session_time"
            filter
            :options="slots"
            optionLabel="key"
            :class="{ 'p-invalid': submitted && !evalate.Session_time }"
            class="w-full"
          />
          <div class="mt-1 mb-5 text-red-500" v-if="error?.specialist_id">
            {{ error.specialist_id[0] }}
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    getDays(id) {
      this.business_hours =
        this.doctors.find((item) => item.id == id)?.business_hours || [];
    },
    ...
    deleteAction() {
      axios
        .delete(`api/evaluations/${this.delete_id}/delete`)
        .then((response) => {
          this.getusers();
          this.deleteDialog = !this.deleteDialog;
        });
  ...
  watch: {
    current_page(newVal, oldVal) {
      axios
        .get(
          `api/child/${localStorage.getItem(
            "child_id",
          )}/get/evaluations?page=${newVal}`,
        )
        .then((response) => {
          const root =
            response.data?.evaluations ||
            ...
            response.data ||
            {};
          this.evaluations = Array.isArray(root.data)
            ? root.data
            : Array.isArray(root)
            ? root
            : [];
